...
# ── 2b. AI assistant rules (shared single-source) ───────────────────
#
# One source of truth in shared/ai-common.md, consumed two ways because the
# two CLIs differ on file inclusion:
#   • Claude Code  — supports @import → CLAUDE.md just imports the sources (live).
#   • Codex CLI    — NO @import (openai/codex#17401) → AGENTS.md is generated by
#                    inlining ai-common.md + codex-tail.md. Re-run install to sync.


def _ai_rule_spec():
    """Return (common, claude_tail, codex_tail, claude_out, codex_out) paths."""
    shared = REPO / "shared"
    return (
        shared / "ai-common.md",
        shared / "claude-tail.md",
        shared / "codex-tail.md",
...
def _ai_banner(srcs):
    return f"<!-- GENERATED by bo-env/install.py — edit shared/{srcs}, then re-run install.py -->"


def _expected_claude_rules(common, claude_tail):
    # Claude Code supports @import; keep sources live by importing them.
    return f"{_ai_banner('ai-common.md / claude-tail.md')}\n@{common}\n@{claude_tail}\n"


def _expected_codex_rules(common, codex_tail):
    # Codex has no @import; inline the shared layer ahead of the Codex-only tail.
    return f"{_ai_banner('ai-common.md / codex-tail.md')}\n\n{common.read_text()}\n\n{codex_tail.read_text()}"


def _check_airules():
    common, claude_tail, codex_tail, claude_out, codex_out = _ai_rule_spec()
    missing = [p.name for p in (common, claude_tail, codex_tail) if not p.exists()]
    if missing:
        return False, f"source(s) missing: {', '.join(missing)}"
    stale = []
    if not claude_out.exists() or claude_out.read_text() != _expected_claude_rules(common, claude_tail):
        stale.append("CLAUDE.md")
    if not codex_out.exists() or codex_out.read_text() != _expected_codex_rules(common, codex_tail):
        stale.append("AGENTS.md")
    if stale:
        return True, f"regenerate: {', '.join(stale)}"
    return False, "CLAUDE.md + AGENTS.md up-to-date"


def _write_generated(dst, content, dry_run):
    """Write generated content to dst, backing up an existing non-matching file."""
    if dst.exists() and not dst.is_symlink() and dst.read_text() == content:
        info(f"{dst}")
        return
    if dry_run:
        info(f"[dry-run] generate {dst}")
        return
...
@step("airules", "2b. AI rules (CLAUDE.md + Codex AGENTS.md)", check=_check_airules)
def step_airules(dry_run=False, **_):
    section("2b. AI Assistant Rules (shared single-source)")
    common, claude_tail, codex_tail, claude_out, codex_out = _ai_rule_spec()
    for p in (common, claude_tail, codex_tail):
        if not p.exists():
            warn(f"source missing: {p} — skipping AI rules")
            return
    _write_generated(claude_out, _expected_claude_rules(common, claude_tail), dry_run)
    _write_generated(codex_out, _expected_codex_rules(common, codex_tail), dry_run)
...
def print_manual_steps():
    ...
    print("  2. Copy ~/d/pab/pab.sparsebundle from encrypted backup, then pab-mount")
